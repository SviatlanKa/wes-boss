...
const filterByBirthYear = inventors.filter(inventor => inventor.year >= 1500 & inventor.year < 1600);
console.table(filterByBirthYear);
...
const allLivesLong = inventors.reduce((total, inventor) => (total + (inventor.passed - inventor.year)), 0);
console.log(allLivesLong);

// 5. Sort the inventors by years lived
const yearLived = [...inventors].sort((a, b) => (a.passed - a.year > b.passed - b.year ? 1 : -1));
console.table(yearLived);

// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name (only work in console)
// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
// const category = document.querySelectorAll('.mw-category .mw-category-group');
// const links = category.querySelectorAll('a');
// const streetsDe = Array.from(links)
...
const sumUpInstances = data.reduce((obj, cur) => {
    (!obj[cur]) ? obj[cur] = 0 : null;
    ++obj[cur];
    return obj;
}, {});
